...
//        auth = FirebaseAuth.getInstance()
    ...
    fun validation(): Boolean {
        ...
        if (first_name.text.toString().trim() { it <= ' ' }.isEmpty()) {
            Toast.makeText(this, "First Name Field is Empty", Toast.LENGTH_SHORT).show()
            return false
        }
        if (last_name.text.toString().trim() { it <= ' ' }.isEmpty()) {
            Toast.makeText(this, "Last Nmae Field is Empty", Toast.LENGTH_SHORT).show()
            return false
        }
